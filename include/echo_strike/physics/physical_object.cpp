    ...
    box.on_collide(
        [this](CollisionBox &other)
        {
            ...
            if (auto wall = dynamic_cast<ObstacleObject *>(other.get_object()))
            {
                ...
                puts("collide obstacle!");
            }
            else if (auto other_obj = dynamic_cast<PhysicaObject *>(other.get_object()))
            {
                float overlap_x = std::min(
                    box_rect.right() - other_box_rect.left(),
                    other_box_rect.right() - box_rect.left());
                ...
                if (overlap_x < overlap_y)
                {
                    ...
                    {
                        box.set_rect(Rect(box_rect.left() - correction, box_rect.down(), box_rect.get_width(), box_rect.get_height()));
                        other.set_rect(Rect(other_box_rect.left() + correction, other_box_rect.down(), other_box_rect.get_width(), other_box_rect.get_height()));
                    }
                    else
                    {
                        box.set_rect(Rect(box_rect.left() + correction, box_rect.down(), box_rect.get_width(), box_rect.get_height()));
                ...
                else
                {
                    ...
                    {
                        box.set_rect(Rect(box_rect.left(), box_rect.down() - correction, box_rect.get_width(), box_rect.get_height()));
                        other.set_rect(Rect(other_box_rect.left(), other_box_rect.down() + correction, other_box_rect.get_width(), other_box_rect.get_height()));
                    }
                    else
                    {
                        box.set_rect(Rect(box_rect.left(), box_rect.down() + correction, box_rect.get_width(), box_rect.get_height()));
                ...
                auto u1 = m_speed, u2 = other_obj->m_speed;
                auto m1 = m_mess, m2 = other_obj->m_mess;

                auto v1 = ((m1 - m2) * u1 + 2 * m2 * u2) / (m1 + m2);
                auto v2 = ((m2 - m1) * u2 + 2 * m1 * u1) / (m1 + m2);

                set_speed(v1);
                other_obj->set_speed(v2);
